...
public class JwtUtil {
    ...
    public String generateAccessToken(Long memberId, UserRole userRole) {
        Date issuedAt = new Date();
        Date expiredAt =
                new Date(issuedAt.getTime() + jwtProperties.accessTokenExpirationMilliTime());
        return buildAccessToken(memberId, userRole, issuedAt, expiredAt);
    }

    public AccessTokenDto generateAccessTokenDto(Long memberId, UserRole userRole) {
        Date issuedAt = new Date();
        Date expiredAt =
                new Date(issuedAt.getTime() + jwtProperties.accessTokenExpirationMilliTime());
        String tokenValue = buildAccessToken(memberId, userRole, issuedAt, expiredAt);
        return new AccessTokenDto(memberId, userRole, tokenValue);
    }
    ...
    public AccessTokenDto parseAccessToken(String token) throws ExpiredJwtException {
        try {
            ...
            return new AccessTokenDto(
                    ...
                    UserRole.valueOf(claims.getBody().get(TOKEN_ROLE_NAME, String.class)),
                    token);
    ...
    private String buildAccessToken(
            Long memberId, UserRole userRole, Date issuedAt, Date expiredAt) {
        return Jwts.builder()
                .setIssuer(jwtProperties.issuer())
                .setSubject(memberId.toString())
                .claim(TOKEN_ROLE_NAME, userRole.name())
